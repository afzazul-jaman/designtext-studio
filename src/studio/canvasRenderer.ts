...
import { TextLayer, BackgroundOverlay } from "./types";
...
function buildTextbox(layer: TextLayer, interactive: boolean): fabric.Textbox {
  ...
  if (layer.effects.glow) {
    tb.set("shadow", new fabric.Shadow({ color: layer.fill, blur: 30, offsetX: 0, offsetY: 0 }));
  }
  if (layer.effects.stroke) {
    tb.set({ stroke: layer.strokeColor, strokeWidth: layer.strokeWidth, paintFirst: "stroke" });
  }
  return tb;
}
...
export async function applyBackground(canvas: fabric.Canvas | fabric.StaticCanvas, opts: RenderOptions) {
  ...
  if (opts.bgMode === "color") {
    ...
    const grad = new fabric.Gradient({
      ...
      colorStops: [
        { offset: 0, color: opts.gradientFrom },
...
export async function renderToDataURL(opts: RenderOptions, multiplier = 1): Promise<string> {
  ...
  for (const layer of opts.layers) {
    const tb = buildTextbox(layer, false);
    staticCanvas.add(tb);
  }
